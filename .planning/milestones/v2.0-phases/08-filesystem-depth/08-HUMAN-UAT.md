...
result: pass
verified: Human-confirmed live 2026-07-14 in an interactive PS7 terminal. User ran `box hash` on 12 System32 DLLs and the stderr progress bar rendered and advanced correctly (`██░░ 1/12 files hashed` … `11/12`, in-place) alongside the digest lines — "the progress bar … work[s] as expected." Structural/functional half was independently re-verified same day (9-file hash + `--json` byte-purity + 17-item flatten, no ANSI on stdout).
