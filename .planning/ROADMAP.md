...
- [x] 04-03-PLAN.md — `ascii`: hand-rolled on image 0.25.10 (artem rejected) — image::open → resize_exact(Triangle) → to_luma8 → dark→light ramp; cols=terminal_width(), rows aspect-corrected /2; monochrome v1; tiny checked-in PNG/JPEG fixtures (D-01/02/03) (ASCI-01) ✓ (2 tasks, TDD RED→GREEN; image::open [extension-based, Pitfall 2] → resize_exact(cols,rows,Triangle) → to_luma8 → as_raw → pure luma_to_char(b" .:-=+*#%@") ramp loop → println! per row; cols=terminal_width() 80-piped INTENTIONALLY diverging from cowsay's fixed width D-02, rows=(cols*h/w/2).max(1) aspect-corrected; monochrome v1 no color path D-03 — ramp emit is the VIS-V2-01 seam; 0-dim guarded with bail! no divide-by-zero; decode error → .with_context → exit-1 no-panic FOUND-05/T-04A-02; image added default-features=false features=["png","jpeg"] RESEARCH A2 trim; 5 unit (luma_to_char extremes/monotonic+bounds, compute_rows square/aspect/clamp) + 4/4 ASCI-01 integration (PNG render, JPEG render, valid-UTF-8 no-ANSI, missing-file exit-1 no-panic) vs tiny 8x8 grayscale-gradient fixtures 140B/340B committed binary; one Rule-3 fix cargo-fmt reflow of two long assert! lines; full suite 111 unit + all integration + clippy -D + fmt clean; ascii stub gone — 1 phase-4 stub remains: matrix)
